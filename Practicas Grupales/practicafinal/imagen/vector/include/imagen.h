...
/**
@brief Sobrecarga del operador + para la Clase Imagen
@param Imagen que se va a sumará a la imagen actual
@retval imagen resultado
*/
   Imagen operator+(const Imagen &img);
...
/**
@brief Funcion que llamara a aArteASCII por cada celda de las @a celdas recibidas por parametro y generara un fichero txt de la imagen convertida en cada una de esas llamadas
@param celdas lista de celdas que contienen la escala de grises para los ficheros txt de salida
@retval true si ha tenido exito en la creacción de los ficheros
*/
   bool listaAArteASCII(const Lista &celdas);

/**
@brief Funcion auxiliar para crear asignar a la @a B la @a A
@param A imagen constante que contiene la imagen que se copiara en @a B
@param B imagen resultante de copiar los datos de @a A
*/
   void copiarImagen(const Imagen &A, Imagen &B);
